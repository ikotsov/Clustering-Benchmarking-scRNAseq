...
def run_preprocessing(accession: str, data_branch: Branch = "pearson", n_pca_components: int = N_PCA_COMPONENTS):
    """
    ...
    data_branch : Branch, default="pearson"
    ...
    # Load & preprocess
    print(
        f"\n=== PREPROCESSING: {accession}, branch={data_branch}, n_pca_components={n_pca_components} ===")
    print()
    ...
    filename = f"{data_branch}_pca_preprocessed.csv.gz"
...
def run_experiment(accession: str, algo_name: str, data_branch: Branch = "pearson"):
    ...
    preprocessed_filename = f"{data_branch}_pca_preprocessed.csv.gz"
    ...
    filename = f"{data_branch}_pca_{algo_name}.csv.gz"
    save_path = os.path.join(output_dir, filename)
